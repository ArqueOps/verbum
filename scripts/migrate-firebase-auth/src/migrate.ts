...
export const SUPPORTED_PROVIDERS = new Set<string>(["password", "google.com", "apple.com"]);
...
export function getPrimaryProvider(user: UserRecord): string | null {
...
export function isAnonymous(user: UserRecord): boolean {
  return (
    !user.providerData ||
...
export async function getAlreadyMigratedUids(supabase: SupabaseClient): Promise<Set<string>> {
...
export function getOAuthProviderId(user: UserRecord, providerName: string): string | null {
  const providerInfo = user.providerData.find((p: { providerId: string }) => p.providerId === providerName);
