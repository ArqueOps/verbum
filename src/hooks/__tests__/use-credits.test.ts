...
let mockProfileData: { study_count: number } | null = { study_count: 10 };
...
describe("useCredits", () => {
  beforeEach(() => {
    ...
    mockProfileData = { study_count: 10 };
  ...
  it("should optimistically decrement creditsRemaining before server confirms", async () => {
    ...
    expect(result.current.creditsRemaining).toBe(9);
    ...
    // Assert
    expect(result.current.creditsRemaining).toBe(8);
  });
  ...
  it("should update creditsRemaining when real-time subscription fires", async () => {
    ...
    act(() => {
      realtimeCallback?.({ new: { study_count: 15 } });
    });

    // Assert
    expect(result.current.creditsRemaining).toBe(15);
  });
