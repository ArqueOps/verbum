// @vitest-environment node
import { describe, it, expect, vi, beforeEach } from "vitest";
import type { SupabaseClient } from "@supabase/supabase-js";
import {
  getAdminMetrics,
  getStudiesPerDay,
  getSubscribersPerDay,
  getCancellationReasons,
} from "../admin-metrics";

function createMockSupabase() {
  const mock = {
    from: vi.fn(),
  };
  return mock as unknown as SupabaseClient & { from: ReturnType<typeof vi.fn> };
}

function mockHeadSelect(count: number) {
  return {
    select: vi.fn().mockReturnValue({
      count,
      data: null,
      error: null,
    }),
  };
}

function mockHeadSelectWithEq(count: number) {
  return {
    select: vi.fn().mockReturnValue({
      eq: vi.fn().mockReturnValue({
        count,
        data: null,
        error: null,
      }),
    }),
  };
}

describe("admin-metrics", () => {
  beforeEach(() => {
    vi.restoreAllMocks();
  });

  describe("getAdminMetrics", () => {
    it("should return correct counts for all fields", async () => {
      const supabase = createMockSupabase();

      supabase.from.mockImplementation((table: string) => {
        if (table === "profiles") {
          return {
            select: vi.fn().mockReturnValue({
              count: 42,
              data: null,
              error: null,
            }),
          };
        }
        if (table === "studies") {
          return {
            select: vi.fn().mockImplementation((_f: string, _opts?: unknown) => {
              return {
                count: 10,
                data: null,
                error: null,
                eq: vi.fn().mockReturnValue({
                  count: 5,
                  data: null,
                  error: null,
                }),
              };
            }),
          };
        }
        if (table === "subscriptions") {
          return {
            select: vi.fn().mockReturnValue({
              eq: vi.fn().mockImplementation((_col: string, val: string) => {
                if (val === "active") return { count: 8, data: null, error: null };
                if (val === "canceled") return { count: 3, data: null, error: null };
                return { count: 0, data: null, error: null };
              }),
            }),
          };
        }
        return { select: vi.fn().mockReturnValue({ count: 0, data: null, error: null }) };
      });

      const result = await getAdminMetrics(supabase);

      expect(result.totalUsers).toBe(42);
      expect(result.totalStudies).toBe(10);
      expect(result.activeSubscriptions).toBe(8);
      expect(result.canceledSubscriptions).toBe(3);
    });

    it("should return zeros when counts are null", async () => {
      const supabase = createMockSupabase();

      supabase.from.mockReturnValue({
        select: vi.fn().mockReturnValue({
          count: null,
          data: null,
          error: null,
          eq: vi.fn().mockReturnValue({
            count: null,
            data: null,
            error: null,
          }),
        }),
      });

      const result = await getAdminMetrics(supabase);

      expect(result.totalUsers).toBe(0);
      expect(result.totalStudies).toBe(0);
      expect(result.totalPublishedStudies).toBe(0);
      expect(result.activeSubscriptions).toBe(0);
      expect(result.canceledSubscriptions).toBe(0);
    });
  });

  describe("getStudiesPerDay", () => {
    it("should group studies by date", async () => {
      const supabase = createMockSupabase();

      supabase.from.mockReturnValue({
        select: vi.fn().mockReturnValue({
          gte: vi.fn().mockReturnValue({
            data: [
              { created_at: "2026-04-10T08:00:00.000Z" },
              { created_at: "2026-04-10T14:30:00.000Z" },
              { created_at: "2026-04-11T09:00:00.000Z" },
            ],
            error: null,
          }),
        }),
      });

      const result = await getStudiesPerDay(supabase, 30);

      const apr10 = result.find((r) => r.date === "2026-04-10");
      const apr11 = result.find((r) => r.date === "2026-04-11");
      expect(apr10?.count).toBe(2);
      expect(apr11?.count).toBe(1);
    });

    it("should return entries with zero counts for days with no studies", async () => {
      const supabase = createMockSupabase();

      supabase.from.mockReturnValue({
        select: vi.fn().mockReturnValue({
          gte: vi.fn().mockReturnValue({
            data: [],
            error: null,
          }),
        }),
      });

      const result = await getStudiesPerDay(supabase, 7);

      expect(result.length).toBe(7);
      for (const point of result) {
        expect(point.count).toBe(0);
      }
    });

    it("should handle null data gracefully", async () => {
      const supabase = createMockSupabase();

      supabase.from.mockReturnValue({
        select: vi.fn().mockReturnValue({
          gte: vi.fn().mockReturnValue({
            data: null,
            error: null,
          }),
        }),
      });

      const result = await getStudiesPerDay(supabase);

      expect(result.length).toBe(30);
      for (const point of result) {
        expect(point.count).toBe(0);
      }
    });

    it("should throw when query returns an error", async () => {
      const supabase = createMockSupabase();

      supabase.from.mockReturnValue({
        select: vi.fn().mockReturnValue({
          gte: vi.fn().mockReturnValue({
            data: null,
            error: { message: "Query failed" },
          }),
        }),
      });

      await expect(getStudiesPerDay(supabase)).rejects.toEqual({ message: "Query failed" });
    });

    it("should return dates sorted ascending", async () => {
      const supabase = createMockSupabase();

      supabase.from.mockReturnValue({
        select: vi.fn().mockReturnValue({
          gte: vi.fn().mockReturnValue({
            data: [],
            error: null,
          }),
        }),
      });

      const result = await getStudiesPerDay(supabase, 5);

      for (let i = 0; i < result.length - 1; i++) {
        expect(result[i]!.date <= result[i + 1]!.date).toBe(true);
      }
    });
  });

  describe("getSubscribersPerDay", () => {
    it("should group subscribers by date", async () => {
      const supabase = createMockSupabase();

      supabase.from.mockReturnValue({
        select: vi.fn().mockReturnValue({
          gte: vi.fn().mockReturnValue({
            data: [
              { created_at: "2026-04-08T10:00:00.000Z" },
              { created_at: "2026-04-08T16:00:00.000Z" },
              { created_at: "2026-04-09T12:00:00.000Z" },
            ],
            error: null,
          }),
        }),
      });

      const result = await getSubscribersPerDay(supabase, 30);

      const apr8 = result.find((r) => r.date === "2026-04-08");
      const apr9 = result.find((r) => r.date === "2026-04-09");
      expect(apr8?.count).toBe(2);
      expect(apr9?.count).toBe(1);
    });

    it("should return entries with zero counts when no subscribers", async () => {
      const supabase = createMockSupabase();

      supabase.from.mockReturnValue({
        select: vi.fn().mockReturnValue({
          gte: vi.fn().mockReturnValue({
            data: [],
            error: null,
          }),
        }),
      });

      const result = await getSubscribersPerDay(supabase, 7);

      expect(result.length).toBe(7);
      for (const point of result) {
        expect(point.count).toBe(0);
      }
    });

    it("should handle null data gracefully", async () => {
      const supabase = createMockSupabase();

      supabase.from.mockReturnValue({
        select: vi.fn().mockReturnValue({
          gte: vi.fn().mockReturnValue({
            data: null,
            error: null,
          }),
        }),
      });

      const result = await getSubscribersPerDay(supabase);

      expect(result.length).toBe(30);
      for (const point of result) {
        expect(point.count).toBe(0);
      }
    });

    it("should throw when query returns an error", async () => {
      const supabase = createMockSupabase();

      supabase.from.mockReturnValue({
        select: vi.fn().mockReturnValue({
          gte: vi.fn().mockReturnValue({
            data: null,
            error: { message: "Connection refused" },
          }),
        }),
      });

      await expect(getSubscribersPerDay(supabase)).rejects.toEqual({ message: "Connection refused" });
    });
  });

  describe("getCancellationReasons", () => {
    it("should group and sort cancellation reasons by count descending", async () => {
      const supabase = createMockSupabase();

      supabase.from.mockReturnValue({
        select: vi.fn().mockReturnValue({
          eq: vi.fn().mockReturnValue({
            not: vi.fn().mockReturnValue({
              data: [
                { cancellation_reason: "Muito caro" },
                { cancellation_reason: "Não uso mais" },
                { cancellation_reason: "Muito caro" },
                { cancellation_reason: "Muito caro" },
                { cancellation_reason: "Não uso mais" },
              ],
              error: null,
            }),
          }),
        }),
      });

      const result = await getCancellationReasons(supabase);

      expect(result).toHaveLength(2);
      expect(result[0]).toEqual({ reason: "Muito caro", count: 3 });
      expect(result[1]).toEqual({ reason: "Não uso mais", count: 2 });
    });

    it("should return empty array when no cancellations exist", async () => {
      const supabase = createMockSupabase();

      supabase.from.mockReturnValue({
        select: vi.fn().mockReturnValue({
          eq: vi.fn().mockReturnValue({
            not: vi.fn().mockReturnValue({
              data: [],
              error: null,
            }),
          }),
        }),
      });

      const result = await getCancellationReasons(supabase);

      expect(result).toEqual([]);
    });

    it("should handle null data gracefully", async () => {
      const supabase = createMockSupabase();

      supabase.from.mockReturnValue({
        select: vi.fn().mockReturnValue({
          eq: vi.fn().mockReturnValue({
            not: vi.fn().mockReturnValue({
              data: null,
              error: null,
            }),
          }),
        }),
      });

      const result = await getCancellationReasons(supabase);

      expect(result).toEqual([]);
    });

    it("should throw when query returns an error", async () => {
      const supabase = createMockSupabase();

      supabase.from.mockReturnValue({
        select: vi.fn().mockReturnValue({
          eq: vi.fn().mockReturnValue({
            not: vi.fn().mockReturnValue({
              data: null,
              error: { message: "DB error" },
            }),
          }),
        }),
      });

      await expect(getCancellationReasons(supabase)).rejects.toEqual({ message: "DB error" });
    });
  });
});
